...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchSongs(page) {
        ...
        loadMoreBtn.style.display = 'none'; // 더보기 버튼 숨기기 (로딩 시)
        ...
        .then(response => {
            ...
            totalSongsCount = data.totalCount; // 서버에서 전체 노래 수 받아오기
            ...
            if (songs.length > 0) {
                ...
                loadMoreBtn.style.display = 'none'; // 더 이상 로드할 노래가 없으면 버튼 숨기기
            }
        ...
        .catch(error => {
            ...
            loading.style.display = 'none'; // 로딩 표시 숨기기
            loadingData = false;
            loadMoreBtn.style.display = 'block'; // 에러 발생 시 버튼 보이기 (재시도 가능하도록)
        });
    }
    ...
    // 더보기 버튼 클릭 시, 현재 페이지에서 표시된 노래 수를 기준으로 더보기 버튼 숨기기
    loadMoreBtn.addEventListener('click', function() {
        // 노래 로드 후 호출되는 함수
        loadMoreSongs().then(() => {
            updateLoadMoreButton(); // 노래 로드 후 버튼 상태 업데이트
        });
    });
    ...
    loadMoreBtn.addEventListener('click', () => {
        fetchSongs(currentPage); // 더보기 버튼 클릭 시 다음 페이지의 노래를 로드
    });
    
    // 초기 데이터 로드
    fetchSongs(currentPage); // 페이지가 로드될 때 처음으로 노래 데이터를 가져옴
