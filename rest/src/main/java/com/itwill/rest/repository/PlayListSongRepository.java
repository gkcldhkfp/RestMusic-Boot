...
public interface PlayListSongRepository extends JpaRepository<PlayListSong, Long> {
    ...
    @Query("SELECT p.playList.pListId, p.song.songId, p.createdTime FROM PlayListSong p WHERE p.playList.pListId = :pListId order by p.createdTime desc")
    List<Object[]> findByPlayListId(@Param("pListId") Long pListId);
	
    @Modifying
    @Transactional
    @Query("DELETE FROM PlayListSong p WHERE p.playList.pListId = :pListId AND p.song.songId = :songId AND p.createdTime = :createdTime")
	void deleteSongByCreatedTime(@Param("pListId") Long pListId, @Param("songId") Long songId, @Param("createdTime") LocalDateTime createdTime);
