...
public interface AlbumRepository extends JpaRepository<Album, Integer> {
	...
	@Query(value = "SELECT * "
	        ...
	        + "        'artist' AS artist_type, "
	        + "        COUNT(DISTINCT al.id) AS like_count "
	        + "    FROM albums a "
	        + "    LEFT JOIN songs s ON a.album_id = s.album_id "
	        + "    LEFT JOIN artist_roles ar ON s.song_id = ar.song_id "
	        + "    LEFT JOIN artists art ON ar.artist_id = art.artist_id "
	        + "    LEFT JOIN album_likes al ON a.album_id = al.album_id "
	        + "    WHERE MATCH(art.artist_name) AGAINST(:keyword IN BOOLEAN MODE) "
	        + "    GROUP BY a.album_id, a.album_name, a.album_image, a.album_type, a.album_release_date, "
	        ...
	        + ") AS combined_results "
	        + "ORDER BY LENGTH(album_name), like_count DESC "
	        + "LIMIT 18 OFFSET :offset", nativeQuery = true)
	...
	@Query(value = "SELECT * "
	        ...
	        + "             g.group_name, g.group_id "
	        + ") AS combined_results "
	        + "ORDER BY album_release_date, like_count DESC "
	        + "LIMIT 18 OFFSET :offset", nativeQuery = true)
	...
	@Query(value = "SELECT * "
	        ...
	        + "             g.group_name, g.group_id "
	        + ") AS combined_results "
	        + "ORDER BY album_name, like_count DESC "
	        + "LIMIT 18 OFFSET :offset", nativeQuery = true)
