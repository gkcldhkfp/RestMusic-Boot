...
CREATE TABLE
	del_users ( -- 탈퇴 회원 (컬럼 2개)
		id 						INT, -- 회원 번호 (5자리)
		deleted_time 			TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 회원 탈퇴 시간 (현재 시간 - 시분초까지 표시함)
...
CREATE TABLE
	comments ( -- 댓글 (컬럼 6개)
		...
		created_time 			TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 작성 시간 (현재 시간 - 시분초까지 표시함)
		modified_time 			TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 수정 시간 (현재 시간 - 시분초까지 표시함)
...
---------------------------------------- 플레이리스트 저장 곡 테이블 ----------------------------------------
CREATE TABLE
	playlist_songs ( -- 플레이리스트 저장 곡 (컬럼 4개)
		playlist_song_id		INT AUTO_INCREMENT, -- 플리 음악 번호 (5자리, 자동 생성)
		p_list_id 				INT, -- 플리 번호 (5자리)
		song_id 				INT, -- 음악 번호 (5자리)
		created_time 			TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 플리 저장 시간 (현재 시간 - 시분초까지 표시함)
		PRIMARY KEY (playlist_song_id), -- 플리 음악 번호 (고유키)
